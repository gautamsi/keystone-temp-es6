...
const startSocketServer_1 = require("./server/startSocketServer");
const startSecureServer_1 = require("./server/startSecureServer");
const startHTTPServer_1 = require("./server/startHTTPServer");
...
const api_1 = require("./lib/middleware/api");
...
const FieldBase_1 = require("./fields/types/FieldBase");
const createApp_1 = require("./server/createApp");
/**
...
class Keystone {
    ...
    */
    ...
    initExpressApp(customApp) {
        ...
        if (customApp) {
            this.app = customApp;
            createApp_1.createApp(this);
        }
        else {
            this.app = createApp_1.createApp(this);
        }
        return this;
    }
    initExpressSession(mongoose) {
        ...
        if (typeof sessionStore === 'function') {
            ...
            const SessionStore = safeRequire_1.safeRequire(sessionStore, this.get('session store') + ' as a `session store` option')(session);
            sessionStorePromise = new Promise(function (resolve, reject) {
    ...
    openDatabaseConnection(callback) {
        ...
        keystone.mongoose.connection.on('error', function (err) {
            ...
            if (mongoConnectionOpen && err && err.name === 'ValidationError')
                return;
            ...
            async.parallel([
                ...
                function (done) {
                    if (ssl === 'only' || unixSocket)
                        return done();
                    startHTTPServer_1.startHTTPServer(keystone, app, function (err, msg) {
                ...
                function (done) {
                    if (!ssl || unixSocket)
                        return done();
                    startSecureServer_1.startSecureServer(keystone, app, function () {
                ...
                function (done) {
                    if (!unixSocket)
                        return done();
                    startSocketServer_1.startSocketServer(keystone, app, function (err, msg) {
    ...
    applyUpdates(callback) {
        this.callHook('pre:updates', (err) => {
            if (err)
                return callback(err);
            updates_1.applyUpdates(this, (err) => {
                if (err)
                    return callback(err);
...
Keystone.Field = FieldBase_1.FieldBase;
Keystone.Storage = storage_1.Storage;
Keystone.content = content_1.Content.init(Keystone.instance);
Keystone.security = {
    csrf: csrf,
};
...
exports.keystone = Keystone.instance;
