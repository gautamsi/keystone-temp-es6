 ...
 */
class LocationType extends FieldTypeBase_1.FieldTypeBase {
    ...
    init() {
        ...
        this.enableMapsAPI = (this.options.enableImprove === true || (this.options.enableImprove !== false && keystone_1.keystone.get('google server api key'))) ? true : false;
    ...
    validateRequiredInput(item, data, callback) {
        ...
        this.requiredPaths.forEach(function (path) {
            ...
            if (input[path] === undefined && currentValue[path])
                return;
    ...
    googleLookup(item, region, update, callback) {
        ...
        doGoogleGeocodeRequest(address, region || keystone_1.keystone.get('default region'), function (err, geocode) {
...
LocationType.properName = 'Location';
...
function doGoogleGeocodeRequest(address, region, callback) {
    ...
    if (keystone_1.keystone.get('google server api key')) {
        options.key = keystone_1.keystone.get('google server api key');
    }
