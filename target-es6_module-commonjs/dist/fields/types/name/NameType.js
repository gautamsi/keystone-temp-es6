 ...
 * @api public
 */
class NameType extends FieldTypeBase_1.FieldTypeBase {
    ...
    getInputFromData(data) {
        ...
        if (first === undefined)
            first = this.getValueFromData(data, '.first');
        let last = this.getValueFromData(data, '_last');
        if (last === undefined)
            last = this.getValueFromData(data, '.last');
    ...
    inputIsValid(data, required, item) {
        ...
        if (!(this.path in data || this.paths.first in data || this.paths.last in data || this.paths.full in data) && item && item.get(this.paths.full))
            return true;
