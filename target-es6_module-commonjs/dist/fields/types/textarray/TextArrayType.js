 ...
 * @api public
 */
class TextArrayType extends FieldTypeBase_1.FieldTypeBase {
    ...
    addFilterToQuery(filter) {
        ...
        if (presence === 'none') {
            query[this.path] = addPresenceToQuery_1.addPresenceToQuery(presence, value);
        }
        else {
            query[this.path] = addPresenceToQuery_1.addPresenceToQuery(presence, {
                $regex: value,
            });
        }
        return query;
...
TextArrayType.properName = 'TextArray';
exports.TextArrayType = TextArrayType;
