 ...
 */
class CloudinaryImagesType extends FieldTypeBase_1.FieldTypeBase {
    ...
    init() {
        ...
        if (!keystone_1.keystone.get('cloudinary config')) {
    ...
    getFolder() {
        let folder = null;
        if (keystone_1.keystone.get('cloudinary folders') || this.options.folder) {
    ...
    addToSchema(schema) {
        ...
        const mongoose = keystone_1.keystone.mongoose;
        const field = this;
        this.paths = {
            // virtuals
        ...
        const folder = function (item) {
            let folderValue = '';
            if (keystone_1.keystone.get('cloudinary folders')) {
                if (field.options.folder) {
                    folderValue = field.options.folder;
                }
                else {
                    const folderList = keystone_1.keystone.get('cloudinary prefix') ? [keystone_1.keystone.get('cloudinary prefix')] : [];
        ...
        const src = function (img, options) {
            if (keystone_1.keystone.get('cloudinary secure')) {
                options = options || {};
                options.secure = true;
            }
     ...
     */
    format(item) {
        return _.map(item.get(this.path), (img) => {
            return img.src();
        }).join(', ');
    }
    /**
    ...
    getOptions() {
        ...
        else if (keystone_1.keystone.get('cloudinary secure')) {
            this.__options.secure = keystone_1.keystone.get('cloudinary secure');
        }
    ...
    updateItem(item, data, files, callback) {
        ...
        function getUploadOptions() {
            ...
            let tagPrefix = keystone_1.keystone.get('cloudinary prefix') || '';
            const uploadOptions = {
                tags: [],
                folder: undefined
            };
            ...
            if (keystone_1.keystone.get('env') !== 'production') {
...
CloudinaryImagesType.properName = 'CloudinaryImages';
exports.CloudinaryImagesType = CloudinaryImagesType;
