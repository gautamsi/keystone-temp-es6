...
const keystone = keystone_1.Keystone.instance;
 ...
 * @api public
 */
class CloudinaryImageType extends FieldTypeBase_1.FieldTypeBase {
    ...
    addSize(options, width, height, other) {
        if (width)
            options.width = width;
        if (height)
    ...
    applySchemaMethod(item, method, ...rest) {
    }
    /**
        ...
        return function () {
            ...
            if (req.body && req.body[paths.select]) {
                ...
                if (typeof imageDelete === 'undefined') {
                    field.applySchemaMethod(item, 'upload', req.files[paths.upload].path, uploadOptions).then(uploadComplete);
                }
                else {
...
CloudinaryImageType.properName = 'CloudinaryImage';
exports.CloudinaryImageType = CloudinaryImageType;
