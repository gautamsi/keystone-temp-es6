 ...
 * @api public
 */
class DatetimeType extends DateType_1.DateType {
    ...
    inputIsValid(data, required, item) {
        if (!(this.path in data && !(this.paths.date in data && this.paths.time in data)) && item && item.get(this.path))
            return true;
...
DatetimeType.properName = 'Datetime';
