...
const FieldTypeBase_1 = require("../fields/types/FieldTypeBase");
const types_1 = require("../fields/types");
const escapeValueForExcel_1 = require("./security/escapeValueForExcel");
...
class List {
    constructor(key, options) {
        ...
        this.schema = new mongoose_1.Schema({}, this.options.schema);
        this.schemaFields = [];
        this.uiElements = [];
    ...
    addFiltersToQuery(filters) {
        ...
        fields.forEach(function (path) {
            ...
            if (!field.addFilterToQuery || !filters[field.path])
                return;
    ...
    apiForGet(options) {
        ...
        return (req, res) => {
            ...
            query.exec(function (err, item) {
                ...
                if (!item)
                    return res.status(404).json({ err: 'not found', id: id });
    ...
    declaresTextIndex() {
        ...
        for (let i = 0; i < indexes.length; i++) {
            ...
            for (let h = 0; h < fieldNames.length; h++) {
                ...
                if (typeof val === 'string' && val.toLowerCase() === 'text')
                    return true;
            }
        }
        return false;
    ...
    field(path, options) {
        ...
        if (!(options.type.prototype instanceof FieldTypeBase_1.FieldTypeBase)) {
            // Convert native field types to their default Keystone counterpart
            if (options.type === String) {
                options.type = types_1.FieldTypes.Text;
            }
            else if (options.type === Number) {
                options.type = types_1.FieldTypes.Number;
            }
            else if (options.type === Boolean) {
                options.type = types_1.FieldTypes.Boolean;
            }
            else if (options.type === Date) {
                options.type = types_1.FieldTypes.Datetime;
            }
            else {
    ...
    getCSVData(item, options) {
        ...
        _.forOwn(rtn, (value, prop) => {
            rtn[prop] = escapeValueForExcel_1.escapeValueForExcel(value);
        });
        return rtn;
    }
    /**
    ...
    register() {
        ...
        this.schema.method('getUpdateHandler', function (req, res, ops) {
            return new updateHandler_1.UpdateHandler(list, this, req, res, ops);
        });
        ...
        this.model.on('index', function (err) {
            if (err)
                console.error("Mongoose model 'index' event fired on '" + list.key + "' with error:\n", err.message, err.stack);
        });
        ...
        this.model.on('index-single-done', function (err, index) {
            if (err)
                console.error("Mongoose model 'index-single-done' event fired on '" + list.key + "' for index:\n", index, '\nWith error:\n', err.message, err.stack);
            else
                debug("Mongoose model 'index-single-done' event fired on '" + list.key + "' for index:\n", index);
        });
        this.model.on('error', function (err) {
            if (err)
                console.error("Mongoose model 'error' event fired on '" + list.key + "' with error:\n", err.message, err.stack);
        });
        return this;
    }
    /**
            ...
            field.validateInput(data, function (valid, detail) {
                if (!valid) {
                    ...
                    if ((field.required || requiredFieldPaths[field.path])
                        && (!field.dependsOn || evalDependsOn_1.evalDependsOn(field.dependsOn, data))) {
