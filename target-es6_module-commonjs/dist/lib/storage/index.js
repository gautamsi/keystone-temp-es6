...
class Storage {
    constructor(options) {
        ...
        for (const path in schemaFields) {
            if (!schemaFields[path])
                continue;
            const type = AdapterType.SCHEMA_TYPES[path] || SCHEMA_TYPES[path];
            if (!type)
                throw Error('Unknown type for requested schema field ' + path);
            schema[path] = type;
        }
    ...
    uploadFile(file, callback) {
        ...
        if (!file)
            return callback(Error('Cannot upload file - No file object provided'));
        // The path to the file is required
        if (!file.path)
            return callback(Error('Cannot upload file - No source path'));
...
function normalizeFile(file, schema, callback) {
    ...
    if (schema.mimetype && !file.mimetype)
        file.mimetype = mime(file.path);
