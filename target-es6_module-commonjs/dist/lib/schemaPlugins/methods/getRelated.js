...
function getRelated(paths, callback, nocollapse) {
    const item = this;
    ...
    _.forEach(paths, function (options) {
        ...
        if (!relationship)
            throw new Error('List.getRelated: list ' + list.key + ' does not have a relationship ' + options.path + '.');
        const refList = keystone_1.keystone.list(relationship.ref);
        if (!refList)
            throw new Error('List.getRelated: list ' + relationship.ref + ' does not exist.');
        const relField = refList.fields[relationship.refPath];
        if (!relField || relField.type !== 'Relationship')
            throw new Error('List.getRelated: relationship ' + relationship.ref + ' on list ' + list.key + ' refers to a path (' + relationship.refPath + ') which is not a relationship field.');
        ...
        if (!item._populatedRelationships)
