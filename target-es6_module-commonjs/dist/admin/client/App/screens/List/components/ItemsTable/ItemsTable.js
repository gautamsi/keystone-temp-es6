...
const ItemsTableDragDrop_1 = require("./ItemsTableDragDrop");
...
class ItemsTable extends React.Component {
    renderCols() {
        ...
        if (!this.props.list.nodelete) {
            cols.unshift(React.createElement("col", { width: constants_1.TABLE_CONTROL_COLUMN_WIDTH, key: "delete" }));
        }
        // add sort col when available
        if (this.props.list.sortable) {
            cols.unshift(React.createElement("col", { width: constants_1.TABLE_CONTROL_COLUMN_WIDTH, key: "sortable" }));
        }
    ...
    render() {
        ...
        if (!items.results.length)
            return null;
        const tableBody = (this.props.list.sortable) ? (React.createElement(ItemsTableDragDrop_1.DragDrop, Object.assign({}, this.props))) : (React.createElement("tbody", null, items.results.map((item, i) => {
            return (React.createElement(ItemsTableRow_1.TableRow, Object.assign({ key: item.id, deleteTableItem: this.props.deleteTableItem, index: i, sortOrder: item.sortOrder || 0, id: item.id, item: item }, this.props)));
        })));
