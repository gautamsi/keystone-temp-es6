...
class TableRow extends React.Component {
    renderRow(item) {
        ...
        let cells = this.props.columns.map((col, i) => {
            let ColumnType = FieldTypes_1.Columns[col.type] || FieldTypes_1.Columns.__unrecognised__; // 'Type'
            let linkTo = !i ? `${Keystone.adminPath}/${this.props.list.path}/${itemId}` : undefined;
            return React.createElement(ColumnType, { key: col.path, list: this.props.list, col: col, data: item, linkTo: linkTo });
        });
        // add sortable icon when applicable
        if (this.props.list.sortable) {
            cells.unshift(React.createElement(ListControl_1.ListControl, { key: "_sort", type: "sortable", dragSource: this.props.connectDragSource }));
        }
        // add delete/check icon when applicable
        if (!this.props.list.nodelete) {
            cells.unshift(this.props.manageMode ? (React.createElement(ListControl_1.ListControl, { key: "_check", type: "check", active: this.props.checkedItems[itemId] })) : (React.createElement(ListControl_1.ListControl, { key: "_delete", onClick: (e) => this.props.deleteTableItem(item, e), type: "delete" })));
        }
        let addRow = (React.createElement("tr", { key: 'i' + item.id, onClick: this.props.manageMode ? (e) => this.props.checkTableItem(item, e) : null, className: rowClassname }, cells));
        if (this.props.list.sortable) {
            return (
...
const dragItem = {
    beginDrag(props) {
        ...
        props.dispatch(actions_1.setDragBase(props.item, props.index));
        return Object.assign({}, send);
    },
    endDrag(props, monitor, component) {
        if (!monitor.didDrop()) {
            props.dispatch(actions_1.resetItems(props.id));
            return;
        }
        ...
        props.dispatch(actions_1.reorderItems(props.item, prevSortOrder, newSortOrder, Number(droppedOn.goToPage)));
    },
};
/**
...
const dropItem = {
    ...
    hover(props, monitor, component) {
        ...
        if (props.rowAlert.success || props.rowAlert.fail) {
            props.dispatch(actions_1.setRowAlert({
                reset: true,
            }));
        }
        ...
        props.dispatch(actions_1.moveItem(dragged, over, props));
...
exports.SortableTableRow = react_dnd_1.DragSource('item', dragItem, dragProps)(react_dnd_1.DropTarget('item', dropItem, dropProps)(TableRow));
