...
Markdown.prototype = {
    ...
    __setListener: function () {
        ...
        let hasRows = typeof this.$textarea.attr('rows') !== 'undefined', maxRows = this.$textarea.val().split('\n').length > 5 ? this.$textarea.val().split('\n').length : '5', rowsVal = hasRows ? this.$textarea.attr('rows') : maxRows;
    ...
    __handle: function (e) {
        let target = $(e.currentTarget), handler = this.$handler, callback = this.$callback, handlerName = target.attr('data-handler'), callbackIndex = handler.indexOf(handlerName), callbackHandler = callback[callbackIndex];
        // Trigger the focusin
    ...
    __localize: function (string) {
        let messages = $.fn.markdown.messages, language = this.$options.language;
    ...
    setFullscreen: function (mode) {
        let $editor = this.$editor, $textarea = this.$textarea;
        if (mode === true) {
    ...
    showEditor: function () {
        let instance = this, textarea, ns = this.$ns, container = this.$element, originalHeigth = container.css('height'), originalWidth = container.css('width'), editable = this.$editable, handler = this.$handler, callback = this.$callback, options = this.$options, editor = $('<div/>', {
            'class': 'md-editor',
            click: function () {
                instance.focus();
            }
        ...
        if (this.$editor === null) {
            ...
            if (container.is('textarea')) {
                ...
                let rawContent = (typeof toMarkdown === 'function') ? toMarkdown(container.html()) : container.html(), currentContent = $.trim(rawContent);
            ...
            if (this.eventSupported('keydown') && typeof jQuery.hotkeys === 'object') {
                editorHeader.find('[data-provider="bootstrap-markdown"]').each(function () {
                    let $button = $(this), hotkey = $button.attr('data-hotkey');
    ...
    parseContent: function () {
        let content, callbackContent = this.$options.onPreview(this); // Try to get the content from callback
    ...
    showPreview: function () {
        let options = this.$options, container = this.$textarea, afterContainer = container.next(), replacementContainer = $('<div/>', { 'class': 'md-editor__preview', 'data-provider': 'markdown-preview' }), content;
    ...
    enableButtons: function (name) {
        let buttons = this.__parseButtonNameParam(name), that = this;
    ...
    focus: function (e) {
        let options = this.$options, isHideable = options.hideable, editor = this.$editor;
    ...
    blur: function (e) {
        let options = this.$options, isHideable = options.hideable, editor = this.$editor, editable = this.$editable;
        if (editor.hasClass('active') || this.$element.parent().length === 0) {
            ...
            if (isHideable) {
                ...
                if (editable.el !== null) {
                    ...
                    let oldElement = $('<' + editable.type + '/>'), content = this.getContent(), currentContent = (typeof markdown === 'object') ? markdown.toHTML(content) : content;
...
$.fn.markdown = function (option) {
    return this.each(function () {
        let $this = $(this), data = $this.data('markdown'), options = typeof option === 'object' && option;
        if (!data)
...
let analyzeMarkdown = function (e) {
    let blurred = false, el, $docEditor = $(e.currentTarget);
