...
import { BlankState, Center, Container, Glyph, GlyphButton, Pagination, Spinner, } from '../../elemental';
...
import { deleteItems, setActiveSearch, setActiveSort, setCurrentPage, selectList, clearCachedQuery, } from './actions';
...
class ListView extends React.Component {
    ...
    renderPagination() {
        ...
        return (React.createElement(Pagination, { currentPage: currentPage, onPageSelect: this.handlePageSelect, pageSize: pageSize, plural: list.plural, singular: list.singular, style: { marginBottom: 0 }, total: items.count, limit: 10 }));
    }
    renderHeader() {
        const items = this.props.items;
        const { autocreate, nocreate, plural, singular } = this.props.currentList;
        return (React.createElement(Container, { style: { paddingTop: '2em' } },
            React.createElement(ListHeaderTitle, { activeSort: this.props.active.sort, availableColumns: this.props.currentList.columns, handleSortSelect: this.handleSortSelect, title: `
						${numeral(items.count).format()}
						${pluralize(items.count, ' ' + singular, ' ' + plural)}
					` }),
            React.createElement(ListHeaderToolbar
            // common
            , { 
                // common
                dispatch: this.props.dispatch, list: listsByPath[this.props.match.params.listId], 
                // expand
            ...
            React.createElement(ListFilters, { dispatch: this.props.dispatch, filters: this.props.active.filters })));
    }
    checkAllTableItems() {
        const checkedItems = {};
    ...
    renderBlankState() {
        ...
        const button = !currentList.nocreate ? (React.createElement(GlyphButton, { color: "success", glyph: "plus", position: "left", onClick: onClick, "data-e2e-list-create-button": "no-results" },
            "Create ",
            currentList.singular)) : null;
        return (React.createElement(Container, null,
            (this.props.error) ? (React.createElement(FlashMessages, { messages: {
                    error: [{
                            title: "There is a problem with the network, we're trying to reconnect...",
                        }]
                } })) : null,
            React.createElement(BlankState, { heading: `No ${this.props.currentList.plural.toLowerCase()} found...`, style: { marginTop: 40 } }, button)));
    }
    renderActiveState() {
        if (this.showBlankState())
            return null;
    ...
    renderNoSearchResults() {
        ...
        return (React.createElement(BlankState, { style: { marginTop: 20, marginBottom: 20 } },
            React.createElement(Glyph, { name: "search", size: "medium", style: { marginBottom: 20 } }),
    ...
    render() {
        if (!this.props.ready) {
            return (React.createElement(Center, { height: "50vh", "data-screen-id": "list" },
                React.createElement(Spinner, null)));
        }
        return (React.createElement("div", { "data-screen-id": "list" },
            this.renderBlankState(),
            this.renderActiveState(),
            React.createElement(CreateForm, { err: Keystone.createFormErrors, isOpen: this.state.showCreateForm, list: this.props.currentList, onCancel: this.closeCreateModal, onCreate: this.onCreate }),
            React.createElement(UpdateForm, { isOpen: this.state.showUpdateForm, itemIds: Object.keys(this.state.checkedItems), list: this.props.currentList, onCancel: () => this.toggleUpdateModal(false) }),
